...
def ingest_tester_dogs_table_func():
    return """
    SELECT * FROM TESTER_DOGS_TABLE WHERE PRODUCT1 = 'Carrot Plushy' OR PRODUCT2 = 'Carrot Plushy'
    """
...
def calculate_product_treats_total_func():
    return """
    SELECT PRODUCT, SUM(TREATS) as TOTAL_TREATS FROM (SELECT PRODUCT1 as PRODUCT, TREATS FROM TESTER_DOGS_TABLE UNION ALL SELECT PRODUCT2 as PRODUCT, TREATS FROM TESTER_DOGS_TABLE) GROUP BY PRODUCT
    """
...
def sum_total_treats_func(calculate_product_treats_total: Table):
    return """
    SELECT SUM(total_treats) FROM {{calculate_product_treats_total}}
    """
...
@dag(
    ...
    catchup=False,
    owner_links={
        "Open in Cloud IDE": "https://cloud.astronomer.io/cl1ntvrrk411461gxsvvitduiy/cloud-ide/cls39zxyq025h01nr13y1jxmc/cls3a0l9c025i01nrcav88zfv",
    },
    tags=["Cloud IDE", "Analysis"],
)
